...
{
    int rv = 0;
    ALOGI("CameraWrapper2: %s", __FUNCTION__);
...
static char * camera2_fixup_getparams(int id __unused, const char * settings)
{
    ...
    if(params.get(KEY_LGE_ISO_MODE)) {
        ...
        ALOGI("%s: ISO mode: %s", __FUNCTION__, isoMode);
                ...
                hw_device_t** device)
{
    ...
    if (name != NULL) {
        ...
        if (rv)
        {
            ALOGE("CameraWrapper2: open_legacy vendor camera open fail");
            goto fail;
        }
